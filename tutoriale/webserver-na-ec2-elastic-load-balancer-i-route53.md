...
Czas na podpięcie naszej domeny pod skrzydła AWS. Aby skierować domenę zakupioną u zewnętrznego dostawcy \(w tym przykładzie, jak już wspomniałem na początku, będzie to [domeny.pl](http://domeny.pl)\) musimy mieć rzecz jasna dostęp do panelu kontrolnego domeny.
...
### Powrót do load balancera

Trochę nam zeszło z tym certyfikatem, ale myślę, że było warto. Czas powrócić do load balancera. Mam nadzieję, że w ferworze walki z **ACM** nie zamknąłeś przypadkowo zakładki, w której rozpoczęliśmy tworzenie **Application Load Balancera**. Jeśli jednak tak się stało, będziesz po prostu musiał powtórzyć kroki z tego rozdziału do momentu, w którym przenieśliśmy się do **Amazon Certficiate Managera**.

Okej, mamy utworzony certyfikat SSL, możemy zatem kliknąć ikonkę do odświeżenia listy certyfikatów i powinien się on na niej znaleźć. Wybieramy certyfikat i klikamy na **Next: Configure Security Groups**. Tutaj tworzymy sobie nową grupę i pozostawiamy wartości domyślne. Klikamy na Next: Configure Routing. W tej sekcji możemy zdefiniować w jaki sposób dany request ma być obsługiwany przez nasz load balancer. Generalnie tworzmy tzw. Target group - powiedzmy, że mamy grupę serwerów webowych, grupę serwerów SQL-owych itd. - dla każdej z tych grup chcemy inaczej reagować na requesty. Na potrzeby tego tutoriala pozostaniemy oczywiście przy prostym przykładzie. W polu nazwa wpisujemy po prostu "WebserverGroup", a resztę wartości pozostawiamy domyślne.
